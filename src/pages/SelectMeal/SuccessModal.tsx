...
export function SuccessModal({ selectedMeals }: { selectedMeals: Record<string, string> }) {
        ...
        <>
            <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} variant={"bottom"}>
                <div className='flex flex-col items-center text-center h-[85vh] w-full'>
                    ...
                    <p className="text-[16px] px-7">The meals you have chosen for the week has successfully been recorded.</p>
                    <div className="w-full flex mt-8 px-4 h-13">
                        <Button label={"View Your Meals For The Week"} onClick={() => {
                            setIsOpen(false);
