...
export function UserActivities() {
    ...
    const carouselItems = useMemo(() => {
        ...
        return days.map(day => {
            const selection = selections.find(s => s.menuDay?.day === day);
            return {
                day,
                mealName: selection?.dayMeal?.meal?.name || "Not Selected",
                // Assuming we don't have meal image in the Selection payload right now, fallback to default
    ...
    return (
        <div className="h-full w-full max-w-md mx-auto bg-white flex flex-col relative overflow-hidden transition-colors duration-500" style={{ backgroundColor: activeBgColor }}>
            {/* Background color extension for top status bar area on iOS */}
            <div className="absolute top-0 left-0 right-0 h-10 -translate-y-full transition-colors duration-500" style={{ backgroundColor: activeBgColor }}></div>
                        ...
                        {carouselItems.map((item, i) => (
                            <SwiperSlide key={i} className="rounded-3xl overflow-hidden shadow-xl bg-white">
                                <div className="w-full h-full relative">
                                    <img src={item.imageUrl} alt={item.mealName} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-in-out hover:scale-105" />
                                    ...
                                    <div className="absolute inset-0 flex flex-col justify-end p-6 text-white pointer-events-none">
                                        ...
                                        <h1 className="text-3xl font-black leading-tight tracking-tight text-white mb-4 drop-shadow-lg line-clamp-2">{item.mealName}</h1>
                                    </div>
                                </div>
                            </SwiperSlide>
                        ))}
